  ...
  const Toggle = ({ label, desc, value, onChange }) => (
    <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'16px'}}>
      ...
      <div onClick={() => onChange(!value)}
        style={{width:'44px', height:'24px', borderRa
cat > ~/Desktop/dataflow/src/Settings.jsx << 'EOF'
import { useState } from 'react'
import { useUser, UserButton } from '@clerk/clerk-react'
    ...
    <div style={{padding:'32px', maxWidth:'700px', margin:'0 auto'}}>
      ...
      <Section title="Profile">
        <div style={{display:'flex', alignItems:'center', gap:'16px', marginBottom:'20px'}}>
          ...
          <div>
            ...
            <div style={{fontSize:'13px', color:'#6b6b80'}}>{user?.primaryEmailAddress?.emailAddress || 'rudhra@datagriid.com'}</div>
          </div>
        </div>
        ...
        <div style={{display:'flex', gap:'12px'}}>
          {['dark', 'light', 'system'].map(t => (
            <button key={t} onClick={() => setTheme(t)}
              style={{padding:'10px 20px', borderRadius:'8px', border: theme === t ? '1px solid #00e5ff' : '1px solid #2a2a38',
                background: theme === t ? 'rgba(0,229,255,0.08)' : '#0a0a0f',
                color: theme === t ? '#00e5ff' : '#6b6b80', cursor:'pointer', fontSize:'14px', fontWeight:'600',
      ...
      <Section title="Plan & Billing">
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'16px'}}>
          ...
          <span style={{padding:'4px 12px', borderRadius:'20px', background:'rgba(0,229,255,0.1)', color:'#00e5ff', fontSize:'12px', fontWeight:'700'}}>
            TRIAL
          </span>
        </div>
      ...
      <button onClick={save} style={{width:'100%', padding:'14px', borderRadius:'10px', border:'none',
        background: saved ? '#00e564' : '#00e5ff', color:'#000', fontWeight:'700', cursor:'pointer', fontSize:'15px', transition:'background 0.3s'}}>
